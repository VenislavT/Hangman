...
int main() {
	...
	while (first_command) {
		...
		if (command == "$ register") {
			...
			collection.add_leaderboard(name_of_leaderboard, new_player,count_of_players,max_count_players, players);
			player = new_player;
		}
			...
			if (input_password != users[user_index].get_password()) {
				...
				for (int i = 0; i < count_of_users; i++) {
					file << users[i].get_username() << " " << users[i].get_password() << " " << users[i].get_email() << endl;
				}
				file.close();
			}
	...
	while (true) {
		...
		/*	collection.print_leaderboard(name_of_leaderboard);*/
			for (int j = 0; j < count_of_players; j++) {
				if (player.get_username() == players[j].get_username()) {
					players[j].set_score(score);
				}
			}
			ofstream lead_file(name_of_leaderboard);
			for (int i = 0; i < count_of_players; i++) {
				lead_file << players[i].get_username() << " " << players[i].get_score() << endl;
			}
			lead_file.close();
			cout << endl;
			cout << endl;
			...
			saved_file.close();
			player.set_score(personal_score);
			/*collection.print_leaderboard(name_of_leaderboard);*/
			for (int j = 0; j < count_of_players; j++) {
				if (player.get_username() == players[j].get_username()) {
					players[j].set_score(personal_score);
				}
			}
			ofstream lead_file(name_of_leaderboard);
			for (int i = 0; i < count_of_players; i++) {
