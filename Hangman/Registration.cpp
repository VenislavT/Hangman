...
int Registration::count_users(string name_of_file) {
    ...
    file.close();
    this->all_users_count = count_of_users-1;
...
void Registration::read_user(const string line, USERS& user) {
    ...
    substring_email = line.substr(second_interval_pos + 1);
    user.set_email(substring_email);

    substring_password = line.substr(first_interval_pos + 1,second_interval_pos - first_interval_pos-1);
...
void Registration::new_password(const int users_count, const string username, USERS* collection_users) {
    ...
    for (int i = 0; i < users_count; i++) {
        if (username == collection_users[i].get_username()) {
            cout << "Username found. Forgotten password." << endl;
            cout << endl;
            cout << "Enter email: ";
            cin >> input_mail;
            bool correct_email1=true;
           /* cout << collection_users[i].get_email();*/
            if (input_mail != collection_users[i].get_email()) {
                correct_email1 = false;
            }
            cout << correct_email1;
            while (!correct_email1) {
